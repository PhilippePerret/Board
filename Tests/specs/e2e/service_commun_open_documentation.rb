...
def run_test
  ...
  Dir.mktmpdir('board-test-project-') do |fixture_dir|
    ...
    #   (ServiceData.js) aplatit ce param en string préfixée "file://", pas
    #   un tableau imbriqué comme les autres services
    wait_until(desc: -> { "carte projet = #{read_project_card(id).inspect}" }) do
      common_services_data = read_project_card(id).dig('common_services_data', 'open-a-file')
      common_services_data.is_a?(Array) && File.realpath(common_services_data[0].sub(%r{\Afile://}, '')) == File.realpath(main_file)
    end
    ...
    sleep 1
    common_services_data = read_project_card(id).dig('common_services_data', 'open-a-file')
    raise "common_services_data modifiée par le 2e clic (rechargement) : #{common_services_data.inspect}" unless File.realpath(common_services_data[0].sub(%r{\Afile://}, '')) == File.realpath(main_file)
  end
ensure
