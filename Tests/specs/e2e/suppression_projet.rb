# Test : suppression réelle d'un projet (bouton "Retirer" de la dialog de
# confirmation, PAS "Archiver" — cf. archivage_projet.rb pour ce cas-là)
...
def run_test
  ...
  # → le projet doit avoir disparu d'appdata.json (ni projects-in, ni
  #   projects-out — contrairement à l'archivage)
  ...
  # → la fiche du projet doit avoir été détruite
  if File.exist?(project_card_path(project_id))
    raise "La fiche du projet existe toujours dans #{PROJECT_CARD_FOLDER}, elle devrait avoir été détruite"
  end
ensure
  remove_fixture_project(project_id) if project_id
end

board_test("suppression réelle d'un projet") { run_test }
