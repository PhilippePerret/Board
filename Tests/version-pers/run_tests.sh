...
# Suite de tests d'intégration de Board — moteur "pers" (un seul
# process osascript, Tests/version-pers/support/ax_server.js en JXA,
# lancé une fois et gardé ouvert par une pipe stdin/stdout pour toute une
# spec, au lieu d'un process osascript relancé à chaque action).
#
...
for spec in "${SPECS[@]}"; do
  ...
  t_start=$(date +%s.%N)
  if output=$(BOARD_TEST_ENGINE=pers ruby "$spec" 2>&1); then code=0; else code=$?; fi
  t_end=$(date +%s.%N)
...
echo "${WHITE}Durée totale (moteur : pers) : ${TOTAL_DUR}s${RESET}"
