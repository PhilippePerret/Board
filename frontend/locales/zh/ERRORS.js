...
const ERRORS = {
    ...
    , 'backend-app-backup-failed': "每日备份失败。"
    , 'backend-app-backup-no-previous': "没有可用的先前备份。"
    , 'backend-app-backup-restore-failed': "恢复先前备份失败。"
    , 'unknown-syntax-file-extension': "扩展名未列在检查表中：$1。"
    , 'invalid-value': "无效值：$1。"
    , 'git-commit-title-erros': "提交过程中发生错误"
    , 'git-status-not-clean': "Git 状态不是干净的。"
    , 'git-status-not-empty': "仍有文件/文件夹需要提交。"
    , 'git-branch-not-main': "应该位于 main 分支上。"
    , 'git-status-added-both-sides': "双方都添加了（内容不同）。"
    , 'git-status-deleted-both-sides': "双方都已删除。"
    , 'git-status-modified-both-sides': "双方都已修改。"
    , 'git-status-add-and-absent': "我方已添加，对方缺失。"
    , 'git-status-absent-and-add': "对方已添加，我方缺失。"
    , 'git-status-deleted-and-modified': "我方已删除，对方已修改。"
    , 'git-status-modified-and-deleted': "我方已修改，对方已删除。"
    , 'git-bad-branch': "您位于错误的 Git 分支上。预期：$1。"
    , 'git-commit-error': "提交文件时发生 Git 错误：$1。"
    , 'git-push-error': "推送提交时发生 Git 错误：$1"
    , 'git-pr-create-error': "创建 Github pull request 时发生 GH 错误：$1。"
    , 'git-pr-waiting-checks-error': "等待检查时发生 GH 错误：$1。"
    , 'git-pr-waiting-checks-failure': "检查过程中发生 GH 错误：一项测试失败。"
    , 'git-unable-checkout-main': "Git 错误：无法返回主分支（$1）。"
    , 'git-unable-pr-merge': "Git 错误：无法合并 Pull Request（$1）。"
    , 'git-commit-init-required': "要在 Github PR 周期中提交您的文件，您必须先启动此周期（主要是：选择一个开发分支）。\n\n如果该分支已经定义但未初始化，您可以在项目数据的 `git_pr_cycle_branche` 属性中指定它。"
    , 'github-pr-cycle-require-clean-status-to-submit': "提交 Github PR 需要状态干净（不应有任何文件待提交）。\n\n请使用上一个服务来完成此操作。"
    , 'git-unable-destroy-branch': "无法删除 Git 分支：$1。"
    , 'github-pr-cycle-branch-should-have-been-deleted': "无法删除开发分支 $1：$2"
    , 'git-init-no-push-permission': "您没有对 Github 仓库 $1 的写入（push）权限。"
    , 'git-init-repo-exists-not-empty': "Github 仓库 $1 已存在且不为空。\n\n您确定这是正确的仓库吗？必须先清空它才能用于此初始化。"
