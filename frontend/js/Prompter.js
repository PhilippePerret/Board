...
class Prompter {
    ...
    if (!value) {
      ...
      const definers = new ParamsDefiner(
        ...
        (definers) => {
          ...
          // Pas de save ici : valeur appliquée en mémoire, le save réel
          // (unique) se fait à la fin de toute la définition du service
          // (cf. ServiceDefiner/Service.js) — jamais pendant.
  ...
  static promptBoolean(spec, callback){
    new ConfirmDialog(Object.assign(this.dialogBase(spec), {
        defaultKey: (spec.actual ?? spec.default) === false ? 'Non' : 'Oui'
