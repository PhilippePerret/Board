...
3. Échecs du run de 12h16 non repris individuellement au-delà de ceux listés ci-dessus : `creation_nouveau_projet_selection_fichier`, `service_commun_edit_documentation`, `service_commun_init_documentation`. `creation_simple_nouveau_projet` et `creation_nouveau_projet_sans_selection_finder` corrigés (session du 2026-07-31) : course entre l'ouverture du socket `TestBridge` et la fin de chargement de `index.html` — `TestBridge.shared.attach(webView:)` déplacé dans `ViewController.webView(_:didFinish:)` (`WKNavigationDelegate`) au lieu d'être appelé juste après la création du `WKWebView`.
   ...
   - `service_commun_update_documentation` : confirmé par Phil que `open .` n'a jamais été voulu (retiré volontairement, pas une régression) — spec corrigée pour ne plus attendre d'ouverture Finder, vérifie `common_services_data` + message de succès.
   - Bug transverse trouvé en creusant : `Project.PROPERTIES` (utilisé dans `ParamDefiner.js` et `Project.js`) n'était plus défini nulle part (remplacé par `PROJECT_DATA`/`TBL_PROJECT_DATA` sans mettre à jour ces 2 usages) — cassait tout param `type: 'project'` avec `if_undefined` dès que la valeur projet était vide (docu-folder, docu-main-file-adoc). Corrigé par Phil (Project.js) + moi (ParamDefiner.js:144, `PROJECT_DATA.some(d => d.id === prop)`).
   - Bug transverse trouvé et corrigé : `assert_service_message_ok!` (`Tests/support/helpers_base.rb`) lisait `#message`, mais `ServiceExecuter#afterRunService` envoie son message de succès via `message(true, ...)` qui affiche dans un `div.exergue-message` séparé (jamais dans `#message`) — le helper ne voyait donc jamais le vrai message et lisait un texte périmé. Ajouté `exergue_message_text` (moteur pont, `Tests/version-pont/support/helpers.rb`) et corrigé le helper pour l'utiliser.
   - `service_commun_ouverture_dossier` + `service_commun_update_documentation` : confirmés passants après ces fixes.
